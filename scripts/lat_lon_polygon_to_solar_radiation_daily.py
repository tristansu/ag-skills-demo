...
This script calculates monthly AVERAGE solar radiation (MJ/m^2/day) by averaging
the value for each day in the month (not just a single representative day).

This script:
1. Takes polygon coordinates (GeoJSON or simple [lon,lat] list)
2. Fetches/generates slope and aspect rasters
3. Calculates daily solar radiation for each day of the year
4. Averages all days in each month
5. Outputs 12 GeoTIFFs (one per month)
...
Usage:
    python scripts/lat_lon_polygon_to_solar_radiation_daily.py \
...
Output:
    {field_id}_solar_jan.tif  - January average (MJ/m^2/day)
    {field_id}_solar_feb.tif  - February average
    ... 
    {field_id}_solar_dec.tif  - December average
...
# Month definitions: (name, days_in_month, start_day_of_year)
MONTHS = [
    ('jan', 31, 1),
    ('feb', 28, 32),
    ('mar', 31, 60),
...
def calculate_daily_radiation(latitude_deg, slope_array, aspect_array, day_of_year):
    """
    Calculate daily solar radiation for a specific day of year.
...
def generate_solar_radiation_daily(polygon_data, output_dir, slope_tif=None, aspect_tif=None,
    ...
    # Precompute latitude in radians for efficiency
    latitude_rad = math.radians(latitude)
    
    # Generate monthly averages by computing daily values
    output_files = {}
    monthly_radiation = {}
    
    for month_name, days_in_month, start_day in MONTHS:
        print(f"  Calculating {month_name} ({days_in_month} days)...")
        
        # Accumulate radiation for all days in this month
        monthly_total = np.zeros_like(slope_array, dtype=np.float64)
        
        for day_of_year in range(start_day, start_day + days_in_month):
            daily_radiation = calculate_daily_radiation(
        ...
        save_float_geotiff(output_path, monthly_average.astype(np.float32), transform, crs="EPSG:4326")
        
        output_files[month_name] = str(output_path)
        monthly_radiation[month_name] = monthly_average
        
        # Print statistics
        valid = monthly_average[~np.isnan(monthly_average)]
        if len(valid) > 0:
    ...
    save_float_geotiff(yearly_path, yearly_average.astype(np.float32), transform, crs="EPSG:4326")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate monthly solar radiation rasters from lat/lon polygon (daily average)"
    )
    parser.add_argument(
        "--input",
        required=True,
    ...
    print("=" * 60)
    print("Monthly Solar Radiation Generation (Daily Average)")
    print("=" * 60)
    ...
    try:
        ...
        result = generate_solar_radiation_daily(
            polygon_data,
            args.output_dir,
