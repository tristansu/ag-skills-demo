#!/usr/bin/env python3
"""Batch process terrain for all fields in a GeoJSON."""
...
TERRAIN_SCRIPT = Path(__file__).parent / "lat_lon_polygon_to_dem_slope_aspect.py"
INPUT_GEOJSON = Path("data/fields_oregon_willamette_ag_2025.geojson")
OUTPUT_DIR = Path("data/terrain")
...
def main():
    ...
    for idx, (_, field) in enumerate(gdf_wgs84.iterrows()):
        ...
        existing = list(OUTPUT_DIR.glob(f"{field_id}_elevation.tif"))
        if existing:
