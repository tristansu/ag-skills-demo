...
def get_field_elevation(bounds, zoom=14):
    ...
    px_min = max(0, int((bounds[0] - tile_bounds.west) / (tile_bounds.east - tile_bounds.west) * 256))
    px_max = min(256, int((bounds[2] - tile_bounds.west) / (tile_bounds.east - tile_bounds.west) * 256))
    py_min = max(0, int((tile_bounds.north - bounds[3]) / (tile_bounds.north - tile_bounds.south) * 256))
    py_max = min(256, int((tile_bounds.north - bounds[1]) / (tile_bounds.north - tile_bounds.south) * 256))
...
def process_fields():
    ...
    for col in ['elevation_min_m', 'elevation_max_m', 'elevation_mean_m', 
                'elevation_std_m', 'elevation_range_m', 'elevation_raster']:
        gdf[col] = None
